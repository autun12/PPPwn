...
//   memcpy((void*)(kbase + sceSblACMgrIsAllowedSystemLevelDebugging_p), "\x31\xC0\xFF\xC0\xC3", 5);
...
//   // Send notification
//   printf("PPPwned: Payload Injected successfully");
...
void stage2(void) {
  ...
  *(uint8_t * )(kbase + kemem_2) = VM_PROT_ALL;
  ...
  // Change directory depth limit from 9 to 64
	...
	kmem[3] = 0x00;
#endif
#if FIRMWARE == 903 // FW 11.00, only neeeded for 11.00
  kmem = (uint8_t *)&kbase[0x233B0DA]; // Move to offsets.h?
  kmem[0] = 0x90;
  kmem[1] = 0x90;
  kmem[2] = 0x90;
  kmem[3] = 0x90;
